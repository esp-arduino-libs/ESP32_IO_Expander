...
public:
     ...
     * @param[in] address I2C device 7-bit address. Should be like `ESP_IO_EXPANDER_I2C_TCA9539_ADDRESS_XX` or
     *                     `ESP_IO_EXPANDER_I2C_TCA9555_ADDRESS_XXX`.
     */
    // TCA95XX_16BIT(int scl_io, int sda_io, uint8_t address)
     ...
     *                     `ESP_IO_EXPANDER_I2C_TCA9555_ADDRESS_XXX`.
     */
    // TCA95XX_16BIT(HostHandle host_handle, uint8_t address)
     ...
     */
    // TCA95XX_16BIT(const Config &config)
    //     : Base(config)
    // {}
     ...
     * @brief Begin object
     *
     * @note  This function typically calls `esp_io_expander_new_i2c_tca95xx_16bit()` to create the IO expander handle.
